...
import AddRecipeModal from "@/components/AddRecipeModal"; // Importujemy modal
...
export default function RecipesPage() {
  ...
  // Definiujemy funkcję pobierania tak, aby była dostępna i dla useEffect, i dla Modala
    ...
    try {
      ...
      const res = await fetch("http://localhost:8000/api/v1/recipes/", {
        headers: {
          ...
          "Cache-Control": "no-cache" // Wymuszamy brak cache'u
        }
      });

      if (res.ok) {
        const data = await res.json();
        console.log("Dane pobrane z API:", data); // Sprawdź to w konsoli F12!
        setRecipes(data);
      }
    } catch (error) {
  ...
  // Wywołujemy pobieranie przy starcie
  useEffect(() => {
    fetchRecipes();
  }, []);
